...
#im plotly
fig = px.bar(transfers_spent, x=transfers_spent.index,\
y=transfers_spent.values, title="Transfers Balance by Confederation")
#change the color of the bars to green if the value is positive and red if the value is negative
#make values logarithmic
fig.update_traces(marker_color=['green' if x >= 0 else 'red' for x in transfers_spent.values], \
